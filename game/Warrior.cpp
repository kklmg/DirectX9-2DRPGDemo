...
CWarrior::CWarrior(int x, int y, DWORD width, DWORD height, CCommonSpriteSheet *image)
{
	...
	CAttribute::Init(&g_pAttMng->GetATTTable(enLOBJ::eHeroWarrior), 1);
...
HRESULT CWarrior::DrawObject(LPDIRECTDRAWSURFACE7 suf, DWORD DrawSetting)
{
	if (m_bIsAlive == FALSE) return FALSE;
	//	UpdateInfo();
	enAction CurAct = GetCurAction();
	enDirection CurDir = GetCurDir();
	//character action animation
	switch (CurAct)//character action 
	{
	case eStand://Idle
	{
...
bool CWarrior::InitAI() 
{
	return TRUE;
}

enAttackStage CWarrior::Attack()
{
